...
import com.katran.gamelogic.PlayRequest;
...
public class SlotGameController {
    ...
    @GetMapping()
    public String showSlotGamePage(Model model, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user != null) {
            model.addAttribute("user", user);
        } else {
            model.addAttribute("user", new User()); // Если не авторизован
        }
        return "slotGame";
    }

    @GetMapping("/fruits")
    public ResponseEntity<List<List<String>>> getFruits(@RequestParam(defaultValue = "3") int rows) {
        List<List<String>> allFruits = Arrays.asList(
                Arrays.asList("🍓", "🍓", "🍓", "🍓", "🍓"),
                Arrays.asList("🍒", "🍓", "🍀", "🐵", "🍌"),
                Arrays.asList("🍒", "🍒", "🍒", "🍒", "🍒"),
                Arrays.asList("🍓", "🍀", "🐵", "🍌", "🍒"),
        ...
        List<List<String>> selectedFruits = allFruits.subList(0, Math.min(rows, allFruits.size()));
        System.out.println("Перемешанный список фруктов: " + selectedFruits);
        return ResponseEntity.ok(selectedFruits);
    }



    public boolean checkWin(List<String> fruits) {
        if (fruits.size() != 15) {
            System.out.println("Error: Invalid number of fruits. Expected 15, got: " + fruits.size());
            return false;
        }
        List<String> secondRow = fruits.subList(5, 10);
        return secondRow.stream().distinct().count() == 1;
    }
    ...
    @PostMapping("/play")
    @ResponseBody
    public ResponseEntity<?> playGame(@RequestBody PlayRequest playRequest, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
        ...
        int betAmount = playRequest.getBetAmount();
        List<String> fruits = playRequest.getFruits();
        if (user.getAmount() < betAmount) {
            // Возвращаем JSON с сообщением об ошибке
            return ResponseEntity.badRequest().body(Map.of("message", "Недостаточно средств!"));
        }

        user.setAmount(user.getAmount() - betAmount);
        boolean isWinner = checkWin(fruits);
        int winnings = 0;
        if (isWinner) {
            winnings = betAmount * 2;
            user.setAmount(user.getAmount() + winnings);
        }

        // Сохраняем изменения и обновляем сессию
        userRepository.save(user);
        session.setAttribute("user", user);

        // Возвращаем JSON с результатами
        return ResponseEntity.ok(Map.of(
                "message", isWinner ? "Вы выиграли " + winnings + " $!" : "Вы проиграли!",
                "win", isWinner,
    ...
    @ResponseBody
    public ResponseEntity<Map<String, Object>> updateProfile(HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
    ...
    public ResponseEntity<?> getCurrentUser(HttpSession session) {
        pageVisitsService.incrementPageVisit("slotGame");
        User user = (User) session.getAttribute("user");
        if (user == null) {
